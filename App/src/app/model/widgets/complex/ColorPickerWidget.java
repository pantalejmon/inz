...
public class ColorPickerWidget extends Widget {
    ...
    public ColorPickerWidget(Project projekt, String Name) throws IOException {
        super(projekt, Name);
    ...
    public void Draw(FXMLDrawPreviewController preview) {
        ...
        preview.DrawRectangle(preview.getTempX() + coX + 9, preview.getTempY() + coY + prefHeight / 2 - 6, 12, 12,
                Color.BLACK);
        preview.DrawRectangle(preview.getTempX() + coX + 10, preview.getTempY() + coY + prefHeight / 2 - 5, 10, 10,
                Color.WHITE);
        preview.getGc().setFill(Color.BLACK);
        preview.getGc().setFont(new Font("DejaVu Sans Mono", 12));
        preview.getGc().fillText("White", preview.getTempX() + coX + 30, preview.getTempY() + coY + prefHeight / 2 + 6,
                prefWidth);
        preview.getGc().setFill(Color.DIMGRAY);
        preview.getGc().fillPolygon(
                new double[] { preview.getTempX() + coX + prefWidth - 20, preview.getTempX() + coX + prefWidth - 10,
                        preview.getTempX() + coX + prefWidth - 15 },
                new double[] { preview.getTempY() + coY + prefHeight / 2 - 5,
                        preview.getTempY() + coY + prefHeight / 2 - 5, preview.getTempY() + coY + prefHeight / 2 + 5 },
                3);
    }

    @Override
    public Element getCode(Document doc) {
        Element element = doc.createElement("ColorPicker");
