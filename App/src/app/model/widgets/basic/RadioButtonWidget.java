...
public class RadioButtonWidget extends Widget {
    ...
    public RadioButtonWidget(Project owner, String name) throws IOException {
        super(owner, name);
        this.name = name;
        this.text = name;
        this.prefHeight = 30;
    ...
    public void Draw(FXMLDrawPreviewController preview) {
        ...
        preview.getGc().strokeOval(preview.getTempX() + coX, preview.getTempY() + coY + prefHeight / 2 - width / 2,
                width, width);
        // preview.getGc().setFill(Color.WHITE);
    ...
    public Element getCode(Document doc) {
        Element element = doc.createElement("RadioButton");
    ...
    @Override
    public Widget copy() {
        RadioButtonWidget w = null;
        try {
            w = new RadioButtonWidget(this.owner, this.name + this.owner.getWidgets().size());
